...
# ssm217基于web技术下的汽车站车辆运管系统开发与设计+vue

![picture](https://raw.githubusercontent.com/GraduationProject-springboot/.github/main/img/wx.png)

### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1gn8XeNE2J?p=16)


# 系统概述
1.1 研究背景

随着计算机技术的发展以及计算机网络的逐渐普及，互联网成为人们查找信息的重要场所，二十一世纪是信息的时代，所以信息的管理显得特别重要。因此，使用计算机来管理汽车站车辆运管的相关信息成为必然。开发合适的汽车站车辆运管系统，可以方便管理人员对汽车站车辆运管系统的管理，提高信息管理工作效率及查询效率，有利于更好的为人们服务。

1.2研究目的

随着互联网技术的快速发展，网络时代的到来，网络信息也将会改变当今社会。各行各业在日常企业经营管理等方面也在慢慢的向规范化和网络化趋势汇合。汽车站车辆运管系统的信息化程度体现在将互联网与信息技术应用于经营与管理，以现代化工具代替传统手工作业。无疑，使用网络信息化管理使信息管理更先进、更高效、更科学，信息交流更迅速。

对于之前汽车站车辆运管系统的管理，大部分都是使用传统的人工方式去管理，这样导致了管理效率低下、出错频率高。而且，时间一长的话，积累下来的数据信息不容易保存，对于查询、更新还有维护会带来不少问题。对于数据交接也存在很大的隐患。如果采用电子化的存储方式就会带来很大的改善，而且给用户的查询带来了很大便利，因此设计一个汽车站车辆运管系统刻不容缓，能够提高信息的管理水平。
## 1.3系统设计思想
...
# 3系统分析
## 3.1可行性分析
通过对本汽车站车辆运管系统实行的目的初步调查和分析，提出可行性方案并对其一一进行论证。我们在这里主要从技术可行性、经济可行性、操作可行性等方面进行分析。
### 3.1.1技术可行性
本汽车站车辆运管系统采用SSM框架，JAVA作为开发语言，是基于WEB平台的B/S架构系统。
...
因此，汽车站车辆运管系统在开发技术上具有很高可行性，且开发人员掌握了一定的开发技术，所以此系统的开发技术具有可行性。
### 3.1.2经济可行性
本汽车站车辆运管系统采用的软件都是开源的，这样能够削减很多的精力和资源，降低开发成本。同时对计算机的配置要求也极低，即使是淘汰下来的计算机也能够满足需要，因此，本系统在经济上是完全具有可行性的，所以在经济上是十分可行的。
### 3.1.3操作可行性
本汽车站车辆运管系统的界面简单易操作，用户只要平时有在用过电脑，都能进行访问和操作。本系统具有易操作、易管理、交互性好的特点，在操作上是非常简单的，因此在操作上具有很高的可行性。

综上所述，此系统开发目标已明确，在技术、经济和操作方面都具有很高的可行性，并且投入少、功能完善、管理方便，因此系统的开发是完全可行的。
## 3.2系统性能分析
### 3.2.1 系统安全性
此汽车站车辆运管系统要严格控制管理权限，具体要求如下：

（1）要想对汽车站车辆运管系统进行管理，首先要依靠用户名和密码在系统中登陆，无权限的用户不可以通过任何方式登录系统和对系统的任何信息和数据进行查看，这样可以保证系统的安全可靠性和准确性。
...
图4.14 站务信息实体属性图
### 4.3.2数据库设计表
汽车站车辆运管系统需要后台数据库，下面介绍数据库中的各个表的详细信息：
...
|bangongshibianhao|varchar(200)|是|NULL|办公室编号|
|bangongshimingcheng|varchar(200)|是|NULL|办公室名称|
|bangongshidizhi|varchar(200)|是|NULL|办公室地址|
|buzhang|varchar(200)|是|NULL|部长|
|bumenzhize|varchar(200)|是|NULL|部门职责|
|gongzuobaogao|varchar(200)|是|NULL|工作报告|
|gongzuojihua|varchar(200)|是|NULL|工作计划|
|gongzuoshizhang|datetime|是|NULL|工作时长|
表4.2 财务管理信息表
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|shijianleixing|varchar(200)|是|NULL|时间类型|
|kaishishijian|date|是|NULL|开始时间|
|jiezhishijian|date|是|NULL|截止时间|
|guanlifeiyong|int(11)|是|NULL|管理费用|
|renyuanfeiyong|int(11)|是|NULL|人员费用|
|qitazhichu|int(11)|是|NULL|其它支出|
|chouchengshouru|int(11)|是|NULL|抽成收入|
|qitashouru|int(11)|是|NULL|其他收入|
|lirun|varchar(200)|是|NULL|利润|
表4.3人力资源管理信息表
...
|gangweiyaoqiu|varchar(200)|是|NULL|岗位要求|
|gangweixingzhi|varchar(200)|是|NULL|岗位性质|
|gongzidaiyu|longtext|是|NULL|工资待遇|
|zhiweiyaoqiu|longtext|是|NULL|职位要求|
|zhaopinrenshu|int(11)|是|NULL|招聘人数|
|gongzuoshijian|datetime|是|NULL|工作时间|
|shangbandidian|varchar(200)|是|NULL|上班地点|
...
|zhandianmingcheng|varchar(200)|是|NULL|站点名称|
|luxian|varchar(200)|是|NULL|路线|
|dizhi|varchar(200)|是|NULL|地址|
|tupian|varchar(200)|是|NULL|图片|
|jingyingfanwei|varchar(200)|是|NULL|经营范围|
|chepiaoleixing|varchar(200)|是|NULL|车票类型|
|guanliyuangonghao|varchar(200)|是|NULL|管理员工号|
|guanliyuanxingming|varchar(200)|是|NULL|管理员姓名|
|chepiaoshoujia|varchar(200)|是|NULL|车票售价|
|chepiaoshuliang|varchar(200)|是|NULL|车票数量|
...
## 5.1 管理员模块的实现
### 5.1.1 员工信息管理
汽车站车辆运管系统的系统管理员可以员工注册信息，可以对员工信息添加修改删除以及查询操作。具体界面的展示如图5.1所示。

![](/md/blog.010.png)

图5.1 员工信息管理界面
### 5.1.2 业务信息管理
系统管理员可以查看对业务信息进行添加，修改，删除以及查询操作。具体界面如图5.2所示。

![](/md/blog.011.png)

图5.3 业务信息管理界面
### 5.1.3 站务信息管理
系统管理员可以对站务信息进行添加，修改，删除以及查询操作。界面如下图所示：
...
## 5.2 用户模块的实现
### 5.2.1 业务信息管理
用户登录后，可以查看业务信息，还可以对业务信息进行查询。界面如下图所示：

![](/md/blog.013.png)

图5.5 业务管理界面
### 5.2.2 办公室管理
用户登录之后，可以查看办公室信息，还可以对办公室信息进行查询。界面如下图所示：

![](/md/blog.014.png)

图5.6 办公室管理界面
### 5.2.3 财务信息管理
用户登录后可以在查看公开的财务信息。界面如下图所示：
